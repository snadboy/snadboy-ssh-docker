...
from .exceptions import DockerCommandError, HostNotFoundError, SSHConnectionError
...
class ConnectionPool:
    ...
    async def execute_docker_command(
        self, host_alias: str, command: str, timeout: Optional[int] = None
    ) -> str:
        ...
        try:
            ...
            if isinstance(
                e, (SSHConnectionError, DockerCommandError, HostNotFoundError)
            ):
                raise
            raise SSHConnectionError(f"Failed to execute command: {e}")

    async def stream_docker_events(
        self, host_alias: str, filters: Optional[Dict[str, str]] = None
        ...
        try:
            ...
            if process.returncode and process.returncode != 0:
                if process.stderr is None:
                    raise DockerCommandError("Event stream error: stderr not available")
