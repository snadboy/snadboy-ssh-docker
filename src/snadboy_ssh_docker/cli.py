...
def list_containers(
    client: SSHDockerClient, host: Optional[str] = None, all_containers: bool = False
) -> None:
    """List containers on specified host or all hosts."""
    try:
        containers = client.list_containers_sync(
            host=host, all_containers=all_containers
        )
        ...
        for container_host, host_containers in by_host.items():
            ...
            for container in host_containers:
                ...
                print(
                    f"  {status} {container['Names']} - {container['Image']} ({container['Status']})"
                )
